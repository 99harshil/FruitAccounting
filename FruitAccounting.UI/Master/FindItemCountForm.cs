...
{
    public partial class FindItemCountForm : BaseSearchForm<ItemCount>
    {
        private readonly ItemCountService _itemCountService;
        private readonly long _companyId;

        public FindItemCountForm(ItemCountService itemCountService, long companyId)
        {
            InitializeComponent();
        ...
        private async void FindItemCountForm_Load(object sender, EventArgs e)
        {
        ...
        protected override async Task LoadAllDataAsync()
        {
            try
            {
                _allData = await _itemCountService.GetAllItemCountsAsync(_companyId);
        ...
        protected override void DisplayData(List<ItemCount> data)
        {
            dgvGroups.Rows.Clear();
            foreach (var count in data)
            {
                dgvGroups.Rows.Add(count.Name, count.ItemGroup?.Name, count.ItemCountId);
            }
        ...
        protected override List<ItemCount> ApplySearchFilter(string searchPattern, bool matchCase)
        {
            RegexOptions options = matchCase ? RegexOptions.None : RegexOptions.IgnoreCase;
            var regex = new Regex(searchPattern, options);

            return _allData.Where(c =>
                regex.IsMatch(c.Name ?? "") ||
                regex.IsMatch(c.ItemGroup?.Name ?? "")
            ).ToList();
        }

        protected override ItemCount? GetSelectedItemFromGrid()
        {
            if (dgvGroups.SelectedRows.Count <= 0)
                return null;

            var row = dgvGroups.SelectedRows[0];
            var itemCountId = Convert.ToInt64(row.Cells[2].Value);
            return _allData.FirstOrDefault(c => c.ItemCountId == itemCountId);
        }

        public ItemCount? SelectedItemCount => SelectedItem;
