...
namespace FruitAccounting.UI
{
    public partial class BankRegisterForm : Form
    {
        ...
        public BankRegisterForm(LedgerService ledgerService, AccountService accountService, DaybookService daybookService,
        ...
        private async void BankRegisterForm_Load(object sender, EventArgs e)
        {
            ...
            _bankDaybooks = await _daybookService.GetDaybooksAsync(_companyId, 'B');

            cmbAccountCode.Items.Clear();
            cmbAccountName.Items.Clear();
            foreach (var daybook in _bankDaybooks.OrderBy(d => d.Name))
            {
        ...
        {
            if (cmbAccountName.SelectedIndex >= 0 && cmbAccountName.SelectedIndex < _bankDaybooks.Count)
            {
                var daybook = _bankDaybooks[cmbAccountName.SelectedIndex];
        ...
        private async Task LoadRegisterAsync()
        {
            ...
            {
                MessageBox.Show("This daybook has no linked account.", "Bank Register");
                return;
            }
            ...
            if (fromDate > toDate)
            {
                MessageBox.Show("From date cannot be after To date", "Bank Register");
                return;
            }
        ...
        {
            MessageBox.Show("Print / PDF export will be added in the next step (FastReport .NET).", "Bank Register");
        }
    }
}
